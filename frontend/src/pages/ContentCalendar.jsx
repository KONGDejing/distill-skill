...
import { Play, Edit3, Check, X, Video, Download } from 'lucide-react'
      ...
      <div className="space-y-3">
        {scripts.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            <p className="text-lg">还没有生成文案</p>
            <p className="text-sm mt-1">选择一个已完成蒸馏的博主来生成今日文案</p>
          </div>
        ) : (
          scripts.map((s) => (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded ${statusColors[s.status] || ''}`}>
                          {s.status === 'pending' ? '待审核' : s.status === 'approved' ? '已通过' : s.status === 'rejected' ? '已驳回' : s.status === 'generated_video' ? '已生成视频' : s.status}
                        </span>
                      </div>
